    ...
    @PostMapping
public ResponseEntity<MessageQueue> createQueue(@RequestBody MessageQueue queue) {
    MessageQueue savedQueue = queueService.createQueue(queue);
    ...
    public ResponseEntity<Void> deleteQueue(@PathVariable("id") long id) {
        if (queueService.deleteQueue(id)) {
            return new ResponseEntity<>(HttpStatus.OK);
        }
        return new ResponseEntity<>(HttpStatus.NOT_FOUND);}

    @GetMapping("/{id}")
    public ResponseEntity<MessageQueue> getQueue(@PathVariable("id") long id) {
        return queueService.findQueueById(id)
            .map(q -> new ResponseEntity<>(q, HttpStatus.OK))
            .orElseGet(() -> ResponseEntity.notFound().build());}
    
    @PutMapping("/{id}")
    public ResponseEntity<MessageQueue> updateQueue(@PathVariable("id") long id, @RequestBody MessageQueue queue) {
        return queueService.findQueueById(id)
            .map(q -> {
                queue.setId(id);
                MessageQueue updatedQueue = queueService.updateQueue(queue);
